...
class Netlink:
    ...
    def sender(self, opponent, ser, stop):
        if ser is None:
            ser = self.modem._serial
    ...
    def netlink_exchange(self, state, opponent, ser = None):
        self.state = state
        if self.state == "connected":
            stop = threading.Event()
            t1 = threading.Thread(target=self.listener, args=(opponent,ser, stop))
            t2 = threading.Thread(target=self.sender,args=(opponent,ser, stop))
    ...
    def serial_poll(self):
        ...
        try:
            ...
            # rather than allowing it to prefix the next command or line.
            SERIAL_FRAGMENT_TIMEOUT = 0.25
            now = time.time()
            if (self.serial_buffer and now - self.serial_buffer_time >= SERIAL_FRAGMENT_TIMEOUT):
                self.logger.info("serial unterminated: %r" % self.serial_buffer)
                # self.logger.info("serial unterminated hex: %s" % binascii.hexlify(self.serial_buffer).decode('ascii'))
            ...
            # Read everything currently available without waiting for a
            # particular line terminator.
            received = self.usb.read(waiting)
            if not received:
                return 0


            # Log exactly what arrived. Do not strip bytes from the hex log.
            # self.logger.info("serial data: %r" % received)
            # self.logger.info("serial hex: %s" % binascii.hexlify(received).decode('ascii'))

            self.serial_buffer += received
            self.serial_buffer_time = now

            while self.serial_buffer:
                cr_pos = self.serial_buffer.find(b'\r')
                lf_pos = self.serial_buffer.find(b'\n')

                terminators = [pos for pos in (cr_pos, lf_pos) if pos >= 0]

                # No complete command line yet.
                if not terminators:
                    break

                line_end = min(terminators)
                full_line = self.serial_buffer[:line_end]

                # Consume all adjacent CR/LF bytes. This supports CR, LF,
                # CRLF, LFCR, and repeated line endings.
                consumed = line_end

                while (
                    consumed < len(self.serial_buffer)
                    and self.serial_buffer[consumed:consumed + 1]
                    in (b'\r', b'\n')
                ):
                    consumed += 1

                self.serial_buffer = self.serial_buffer[consumed:]

                # A split CRLF can leave an empty line on the next poll.
                if not full_line:
                    continue
                ...
                # Recognized as a Hayes command, but not one needing
                # special handling.
                self.usb.write(b'OK\r\n')
                self.logger.info("OK")
    ...
    def netlink_server(self, ser = None):
        ...
        if server:
            ...
            if server_type == 'transparent':
                self.netlink_transparent_server(server, ser)
            else:
                self.netlink_standard_server(server, ser)
    ...
    def netlink_transparent_server(self, server_cfg, ser = None):
        ...
        if ser is None:
            # We're using a modem connection. Establish a serial link by answering.
            ser = self.modem._serial
            self.modem.stop_dial_tone()
        ...
        def serial_reader():
            ...
            while not stop.is_set():
                try:
                    ...
                    modem_tail[0] = (modem_tail[0] + data)[-64:]
                    ...
                    if escape_deadline is not None:
                        ...
                        escape_buffer = (escape_buffer + data)[-64:]

                    else:
                        # Retain two bytes so +++ can span multiple serial reads.
                        scan = marker_tail + data
                        escape_pos = scan.find(b'+++')

                        if escape_pos >= 0:
                            self.logger.info(
                                "%s: MODEM ESCAPE SEQUENCE",
                                label
                            )
                    ...
                    if escape_deadline is not None:
                        ...
                        if b'ATH0' in normalized or b'ATZ0' in normalized:
                            self.logger.info("%s: DETECTED MODEM HANGUP: %r", label, escape_buffer)
                            stop.set()
                            break

                        if now >= escape_deadline:
                            self.logger.info("%s: MODEM ESCAPE WINDOW EXPIRED", label)
                            escape_deadline = None
                            escape_buffer = b''
                            marker_tail = b''
    ...
    def netlink_standard_server(self, server_cfg, ser = None):
        """
    ...
    def dcnet_connect(self, ser = None):

        if ser is None:
            device_name = self.modem.device_name
            device_speed = self.modem.device_speed
            self.modem.stop_dial_tone()
            if not self.modem_answer():
                self.reset()
                return
        else:
            device_name = str(ser.port)
            device_speed = str(ser.baudrate)
        time.sleep(3)
        ...
        cmd = [path, "-t", "{}".format(device_name), "-b", "{}".format(device_speed), "-p", self.dcnet_port]
